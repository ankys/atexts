...
$N$次元空間上の2点$P = (x_1, dots, x_N)$, $Q = (y_1, dots, y_N)$を取ると成分同士を引くことで新しい実数の組
$
...
特に有界閉区間に当たる有界閉矩形が重要であり、$2 N$個の実数$a_1 <= b_1, dots, a_N <= b_N$で定まる点集合
$
...
- ${ x in RR^N mid(|) f(x) eq.not 0 } = f^(-1) (RR\\{0})$.
