...
$
norm(f)_(L^p (X, m; K)) := (integral_X abs(f)^p dd(m))^(1/p) < oo
$
を満たすもの全体は後述するように半ノルム空間をなし、
誘導するノルム空間を$L^p (X, m; K)$とおき$p$を次数とする_ルベーグ空間_と呼ぶ。
ここで測度$m$や空間$X$および$K$はしばしば省略される。
さらにノルムもしばしば$norm(f)_p$と書かれることがある。
...
#proof[
まず、半ノルムであることのうち非負値性と斉次性はすぐにわかるので詳細は省略する。
三角不等式はミンコフスキーの不等式から従う。TODO
完備性はリース＝フィッシャーの定理から従う。TODO
]
