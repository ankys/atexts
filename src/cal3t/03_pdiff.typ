...
#definition([偏微分])[
$f(x_1, ... x_N)$を$RR^N$の部分集合$X$上で定義された$N$変数実数値関数、$i = 1, ....N$とする。

$(a_1, ... a_N)$を$X$の内点、つまり半径$r > 0$が存在して$B_r (a) subset X$が成り立つとする。
ここで他の成分は固定して第$i$成分だけ動かした時の微分係数
$
lim_(x_i -> a_i) (f(a_1, ... a_(i-1), x_i, a_(i+1), ....a_N)-f(a_1, ....._(i-1), a_i, a_(i+1), ......N))/(x_i-a_i)
$
が存在するとき$f$は点$(a_1, ... a_N)$で$x_i$について_偏微分可能_であるといい、極限を_偏微分係数_という。
関数$y = f(x_1, ... x_N)$の点$(a_1, ....a_N)$で$x_i$についての偏微分係数を
$
f_(x_i) (a_1, ... a_N),
quad pdv(f, x_i) (a_1, ... a_N),
quad evaluated(pdv(f, x_i))_((x_1, ... x_N) = (a_1, ....a_N)), \
evaluated((f(x_1, ... x_N))_(x_i))_((x_1, ....x_N) = (a_1, ....._N)),
quad evaluated(pdv(, x_i)(f(x_1, ... x_N)))_((x_1, ....x_N) = (a_1, ....._N)), \
evaluated(y_(x_i))_((a_1, ... a_N)),
quad evaluated(pdv(y, x_i))_((a_1, ... a_N))
$
などで表す。

$X$を開集合、つまり$X$の点がすべて内点である場合を考える。
$X$のすべての点で関数$f$が$x_i$について偏微分可能であるとき$f$は$X$上$x_i$について偏微分可能であるという。
この時、$(x_1, ... x_N) in X$に対して偏微分係数$f_(x_i) (x_1, ....x_N)$を対応させる集合$X$上の実数値関数を$f$の$x_i$についての_偏微分導関数_という。
$y = f(x_1, ... x_N)$の$x_i$についての偏微分導関数を
$
f_(x_i),
quad pdv(f, x_i),
quad (f(x_1, ... x_N))_(x_i),
quad pdv(, x_i)(f(x_1, ... x_N)),
quad y_(x_i),
quad pdv(y, x_i)
$
などで表す。
...
方向$x_i$と点$(a_1, ... a_N)$を決めるごとに偏微分係数は実数であり、
...
$
の$x$偏微分を考える。
$y$を固定するごとに$x$について微分すると、$(x, y) != (0, 0)$においては偏微分可能で
$
f_x (x, y) = x/sqrt(x^2+y^2)
$
となる。
$(x, y) = (0, 0)$においては$f(x, 0) = abs(x)$なので、これは$x$について偏微分可能でない。
同様にして$f(x, y)$の$y$偏微分は$(x, y) != (0, 0)$においては偏微分可能で
$
...
$
を考える。
この関数は少なくとも$(x, y) != (0, 0)$においては偏微分可能で商の微分などから
$
...
#definition([勾配])[
$N$変数関数$f(x) = f(x_1, ... x_N)$が点$x = a$において各$x_i$ ($i = 1, ....N$)について偏微分可能であるとするとき、
偏微分係数を順番に並べて得られる$N$次元ベクトル
$
gradient f(a) = (f_(x_1) (a), ... f_(x_N) (a))
$
...
テキストによっては全微分
$
dd(f) = f_(x_1) (a) dd(x_1)+...f_(x_N) (a) dd(x_N)
$
というものを導入することがある。
...
$
であったが、この多変数関数版を考えたい。
$f(x)$が多変数関数$f(x) = f(x_1, ... x_N)$の場合は$g(f(x))$も多変数関数で偏微分を考えると
$
(g(f(x)))_(x_i) = g'(f(x))f_(x_i) (x),
quad gradient(g(f(x))) = g'(f(x))gradient f(x)
$
となるが、
問題は$g(y)$が多変数関数$g(y) = g(y_1, ... y_M)$の場合である。
このとき$f(x)$は$RR^M$値関数$f(x) = (f_1 (x), ... f_M (x))$である必要があり、
$g(f(x))$は（$f(x)$が一変数関数ならば）一変数関数となるのでその微分を考える価値がある。
この関数$f(x)$が$x = a$で微分可能であるとは各$f_i (x)$ ($i = 1, ... M$)が$x = a$で微分可能であることをいい、
$
f'(a) = (f_1 '(a), ... f_M '(a))
$
と定義する。
...
#proposition([合成関数の微分])[
$RR^M$値関数$f(x) = (f_1 (x), ... f_M (x))$が点$x = a$で微分可能で$M$変数関数$g(y) = g(y_1, ....y_M)$が点$y = f(a)$で全微分可能とする。
この時、合成関数$f(g(x))$も$x = a$で微分可能で、
$
evaluated((g(f(x)))')_(x = a)
&= gradient g(f(a)) dot f'(a) \
&= g_(y_1)(f(a))f_1 '(a)+...g_(y_M)(f(a))f_M '(a)
$
が成立する。
]
...
#definition([連続微分可能性])[
$RR^N$の開部分集合$X$上の実数値関数$f$が各変数$x_i$ ($i, 1, ... N$)に対して偏微分可能で偏微分導関数$f_(x_i)$が$X$上の連続関数である時、$f$は$X$上_連続微分可能_であるという。
]
...
#proof[
証明は点$a = (a_1, ... a_N) in X$とその近傍の点$x = (x_1, ....x_N) in X$について、$x$から$a$へ$x_1, ....._N$軸に沿って近づけるという方法で行う。
そのためにまず各$x_i$に対して次の値を評価する。
$
abs(f(x_1, ... x_i, ....x_N)-f(x_1, ....._i, ......N)-f_(x_i) (a_1, ...,..., ..., ...(x_i-a_i))
$
ここで$x |-> f(x_1, ... x, ....x_N)$に対する平均値の定理より$0 <= theta <= 1$が存在して、
$
&abs(f(x_1, ... x_i, ....x_N)-f(x_1, ....._i, ......N)-f_(x_i) (a_1, ...,..., ..., ...(x_i-a_i)) \
&quad <= abs(f_(x_i) (x_1, ... theta a_i+(1-theta)x_i, ....x_N)-f_(x_i) (a_1, ....._i, ......N))(x_i-a_i)).
$
したがって、$omega$を$gradient f$の連続性の度合いとして、
$
&abs(f(x_1, ... x_i, ....x_N)-f(x_1, ....._i, ......N)-f_(x_i) (a_1, ...,..., ..., ...(x_i-a_i)) \
&quad <= omega(norm((x_1, ... theta a_i+(1-theta)x_i, ....x_N)-(a_1, ....._i, ......N)))abs(x_i-a_i) \
...
$
&abs(f(x)-f(a)-gradient f(a) dot (x-a)) \
&quad <= sum_(i = 1)^N abs(f(a_1, ... x_i, ....x_N)-f(a_1, ....._i, ......N)-f_(x_i) (a_1, ...,..., ..., ...(x_i-a_i)) \
...
#proof[
@t_pcurv より$X$の任意の二点$x, y$は$x_1, ... x_N$軸に並行ないくつかの線分で結ばれて、それぞれの線分で偏微分が$0$なので、$f(x) = f(y)$が成り立つ。
]
...
#definition([高階偏微分])[
$RR^N$の開部分集合$X$上の関数$f$が変数$x_i$ ($i = 1, ... N$)に対して偏微分可能で偏微分導関数$f_(x_i)$が変数$x_j$ ($j = 1, ....N$)で偏微分可能である時、$f$は$X$上_二回偏微分可能_であるといい、偏微分導関数
$
...
を$f$の_二階偏微分導関数_という。
同様にして、$i_1, ... i_n = 1, ....N$
($n = 1, 2, 3, ...)に対して偏微分導関数$f_(x_(i_1) ...._(i_n))$を$f$の_$n$階偏微分導関数_という。
すべての$n$階偏微分導関数が連続の時、$f$は$X$上で_$C^n$級_であるという。
さらに$f$が$X$上ですべての$n = 1, 2, 3, ...で$C^n$級である時、$f$は_$C^oo$級_であるという。
]
...
#definition([ヤコビ行列])[
$N$次元点集合上の$M$個の実数値関数$f^1 (x) = f^1 (x_1, ... x_N), ....f^M (x) = f^M (x_1, ....._N)$が内点$x = a$で偏微分可能な時、$RR^M$値関数$F(x) = (f^1 (x), ......M (x))$は$x = a$で偏微分可能といい、
$M times N$型行列
$
J F(a)
= gradient F(a)
= mat(f^1_(x_1) (a), ...c, f^1_(x_N) (a); ...., .....wn, ......f^M_(x_1) (a), .......^M_(x_N) (a))
$
...
#definition([ヘッセ行列])[
$N$次元点集合上の実数値関数$f(x) = f(x_1, ... x_N)$が内点$x = a$の近傍で二回偏微分可能な時、$N$次正方行列
$
H f(a)
= gradient^2 f(a)
= mat(f_(x_1 x_1) (a), ...c, f_(x_1 x_N) (a); ...., .....wn, ......f_(x_N x_1) (a), ......._(x_N x_N) (a))
$
...
#remark[
正方行列$A$が任意のベクトル$e != 0$に対して$e dot A e > 0$を満たすことは$A$が正定値であると呼ばれ、$A$の固有値がすべて正であるとも表現される。
...
ここで$f_y (a, b) != 0$の時、ある半径$r, R > 0$が存在して、各$x in B_r (a)$に対して$y in B_R (b)$の範囲で$f(x, y) = 0$を満たす$y$が一意に存在しそれを$g(x)$とおくと$g$は$C^1$級であり、
$
g'(x) = -frac(f_x (x, g(x)), f_y (x, g(x)))
$ <e_imdiff>
が成り立つ。
...
実際には$gradient g(a) != 0$という条件もつくことに注意する。
...
#proof[
$gradient g(a, b) != 0$つまり$g_x (a, b) != 0$または$g_y (a, b) != 0$の時を考えて、変数$x, y$の対称性より$g_y (a, b) != 0$の場合を考える。
