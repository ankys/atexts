...
しかしながら空間が何かと聞かれたら幾何学的な問題となり難しいので、本テキストでは逆に$N = 1, 2, 3, ...として$N$個の実数$x_1, ....x_N$の組$P = (x_1, ....._N)$のことを$N$次元空間上の_点_と呼び、$N$次元空間上の点をすべて集めたものを$N$次元_空間_ということにする。
...
$N$次元空間上の点$P = (x_1, ... x_N)$に対して、実数$x_i$を$P$の_第$i$成分_という。
ただし、$i = 1, ... N$である。
...
つまり、二点$P = (x_1, ... x_N)$, $Q = (y_1, ....y_N)$間の_距離_$d(P, Q)$をピタゴラスの定理（三平方の定理）にもとづき
$
d(P, Q)
= d((x_1, ... x_N), (y_1, ....y_N))
...
$N$次元空間上の2点$P = (x_1, ... x_N)$, $Q = (y_1, ....y_N)$を取ると成分同士を引くことで新しい実数の組
$
arrow(P Q) = (y_1-x_1, ... y_N-x_N)
$
が定義できる。
...
実際、ベクトル$u = (u_1, ... u_N)$と$v = (v_1, ....v_N)$に対して加法
$
u+v = (u_1+v_1, ... u_N+v_N)
$
が定義でき、ベクトル$u = (u_1, ... u_N)$と実数$a$に対して定数乗法
$
a u = (a u_1, ... a u_N)
$
が定義できる。
また、2つのベクトル$u = (u_1, ... u_N)$と$v = (v_1, ....v_N)$に対して、
$
u dot v = u_1 v_1+...u_N v_N
$
を$u$と$v$の_内積_といい、
$
norm(u) = sqrt(u_1^2+...u_N^2)
$
...
空間ベクトル$u = (u_1, ... u_N)$と$v = (v_1, ....v_N)$に対して、距離をその差のノルムとする、つまり
$
d(u, v) = norm(v-u) = sqrt((v_1-u_1)^2+...(v_N-u_N)^2)
$
...
空間上の点$P = (x_1, ... x_N)$と空間ベクトル$u = (u_1, ....u_N)$に対して$P$を$u$だけ_平行移動_して空間上の点
$
P+u = (x_1+u_1, ... x_N+u_N)
$
を得ることができる。
...
特に有界閉区間に当たる有界閉矩形が重要であり、$2 N$個の実数$a_1 <= b_1, ... a_N <= b_N$で定まる点集合
$
[a_1, b_1] times ...times [a_N, b_N]
$
を_有界閉矩形_と呼ぶ。

点集合$X$を考える。 つまり、$X$は$N$次元空間$RR^N$の部分集合である。
ここで実数$a_1 <= b_1, ... a_N <= b_N$が存在して$X subset [a_1, b_1] times ....imes [a_N, b_N]$が成り立つとき、$X$は_有界_であるという。
つまり$X$を被覆する有界閉矩形が存在すると言い換えることができる。

一般的な教科書では有界性を円で定義されることが多いが、矩形で定義しても円で定義しても同じである。

#proposition[
点集合$X$に対して、$X subset [a_1, b_1] times ...times [a_N, b_N]$となる有界閉矩形$[a_1, b_1] times ....imes [a_N, b_N]$が存在することと$X subset overline(B)_R (0)$となる半径$R >= 0$が存在することは同値である。
ただし、$0 = (0, ... 0)$である。
]

#proof[
有界閉矩形$[a_1, b_1] times ...times [a_N, b_N]$で被覆できたとすると$L = max{ abs(a_1), abs(b_1), ....abs(a_N), abs(b_N) }$として有界閉矩形$[-L, L]^N$でも被覆でき、$R = sqrt(N)L$とすればよい。
...
$[macron(a)_1, macron(b)_1] times ...times [macron(a)_N, macron(b)_N]$は有界閉矩形であり、$X subset [macron(a)_1, macron(b)_1] times ....imes [macron(a)_N, macron(b)_N]$であり、$X subset [a_1, b_1] times .....mes [a_N, b_N]$ならば$[macron(a)_1, macron(b)_1] times ......es [macron(a)_N, macron(b)_N] subset [a_1, b_1] times ... ...s [a_N, b_N]$を満たす。
$
...
つまり、点列$(a_n)$が$n -> oo$で点$a$に収束するとは
#quote(block: true)[
...
#definition([点列の極限])[
点列$(a_n)$が$n -> oo$で点$a$に_収束する_あるいは$a$が$(a_n)$の$n -> oo$での_極限である_とは$n -> oo$で
$
d(a, a_n) -> 0
$
が成り立つことをいう。
点列$(a_n)$が何らかの数に収束する時この点列は_収束する_といい、そうでない時_発散する_という。
]

#example[
$a$を定点として定点列$(a)_n$の極限は$a$である。
]
...
#proposition[
$(a_n)$を点列、$a$を点とする。
もし$(a_n)_n$の任意の部分列$(a_(n_k))_k$が$a$に部分列収束する、つまり$a$に収束する部分列$(a_(n_(k_l)))_l$が存在するとき、$(a_n)$は$a$に収束する。
]

#proof[
非負の実数列$(d(a, a_n))_n$を考えると実数列での対応する命題が成り立つので、$d(a, a_n) -> 0$であり証明が完了する。
]

== 連続関数

この節では主に多変数関数$f: RR times ...times RR -> RR$の極限や連続性について見ていく。
このとき直積$RR times ...times RR$を$N$次元空間$RR^N$と同一視すると関数$f$は空間上の点からの一変数関数のように見なすことができ、点列の収束のところで見たように実数だったら絶対値を使っていたところを距離に置き換えることで、極限が定義できる。

より詳しくは$N = 1, 2, 3, ...として、$N$次元点集合$X$上で定義された実数値関数$f: X -> RR$を考える。
...
今後の議論の展開の中で値域が実数でなく$RR^M$ ($M = 1, 2, 3, ...)である場合に拡張する必要がありそのために極限の概念を拡張しておく。
...
#proposition[
$f_1, ... f_M$を点集合$X$上の実数値関数として$F(x) = (f_1 (x), ....f_M (x))$により$RR^M$値関数$F$を定める。
$a$を$X$の極限点、$L = (l_1, ... l_M)$を$RR^M$の点とする。
このとき、関数$F(x)$が$x -> a$で点$L$に収束することは、
各$i = 1, ... M$に対して関数$f_i (x)$が$x -> a$で実数$l_i$に収束することと同値である。
]

#proof[
$F(x)$が収束するならばの方が簡単で、各$i = 1, ... M$に対して
$
...
新しい度合い関数を$omega_a (r) = max{ omega_a^1 (r), ... omega_a^M (r) }$として取ると、
$
d(L, F(x))
&= sqrt((f_1 (x)-l_1)^2+...(f_M (x)-l_M)^2) \
&<= sqrt(omega_a^1 (d(a, x))^2+...omega_a^M (d(a, x))^2)
...
$N$変数関数は点$(a_1, ... a_N)$から点$(x_1, ....x_N)$の距離が
$
d((a_1, ... a_N), (x_1, ....x_N)) = sqrt((x_1-a_1)^2+....._N-a_N)^2)
$
で与えられることから以下のようにして定義される。

#definition([多変数関数の極限])[
$f$を$RR^N$の部分集合$X$上で定義された$N$変数実数値関数、$(a_1, ... a_N)$を$X$の極限点、$l$を実数とする。
$N$変数関数$f(x_1, ... x_N)$が$(x_1, ....x_N) -> (a_1, ....._N)$で実数$l$に_収束する_あるいは$l$が$f(x_1, ......N)$の$(x_1, ...,...) -> (a_1, ..., ...$での_極限である_とは
度合い関数$omega_((a_1, ... a_N))$が存在し任意の$(x_1, ....x_N) in X$に対して
$
abs(f(x_1, ... x_N)-l) <= omega_((a_1, ....a_N)) (sqrt((x_1-a_1)^2+....._N-a_N)^2))
$
が成り立つことをいう。
]
...
$f$を$RR^N$の部分集合$X$上で定義された$N$変数実数値関数、$(a_1, ... a_N)$を$X$の極限点であり$X$は$(a_1, ....a_N)$を元として含むとする。
ここで$f(x_1, ... x_N)$が$(x_1, ....x_N) -> (a_1, ....._N)$で$f(a_1, ......N)$に収束する、つまり
$
lim_((x_1, ... x_N) -> (a_1, ....a_N)) f(x_1, ....._N) = f(a_1, ......N) $
の時、関数$f(x_1, ... x_N)$は点$(x_1, ....x_N) = (a_1, ....._N)$であるいは関数$f$は点$(a_1, ......N)$で_連続_であるという。
]

#definition([多変数連続関数])[
$f$を$RR^N$の部分集合$X$上で定義された$N$変数実数値関数として、$X$の点はすべて$X$の極限点であるとする。
ここで、すべての点$(a_1, ... a_N) in X$で$f$が連続である時、関数$f$は集合$X$上の_連続関数_であるという。
]

#remark[
以上のことをまとめると$N$変数実数値関数$f(x_1, ... x_N)$が$(x_1, ....x_N) = (a_1, ....._N)$で連続であることは度合い関数$omega_((a_1, ......N))$が存在し
$
abs(f(x_1, ... x_N)-f(a_1, ....a_N)) <= omega_((a_1, ....._N)) (sqrt((x_1-a_1)^2+......N-a_N)^2))
$
が成り立つことと同値である。
]
...
#proposition([射影の連続性])[
$i = 1, ... N$として、以下の射影は連続関数である。
$
proj_i: x_1 in RR, ... x_N in RR |-> x_i in RR.
$
]
...
不等式
$
abs(x_i-a_i) <= sqrt((x_1-a_1)^2+...(x_N-a_N)^2).
$
が成り立つことからすぐわかる。
]
...
ここで$n = 1, 2, 3, ...ごとに$delta = 1/n$を考えて上記の条件を満たす$x$が存在することからそのうち一つを$a_n$とおく。
...
- $A$を閉集合として、補集合$A^c$上の点$a in A^c$を考える。
  ...
  ここで$n = 1, 2, 3, ...ごとに$r = 1/n$を考えて上記の条件を満たす$x$が存在することからそのうち一つを$a_n$とおく。
...
#proof[
有界閉矩形$[a_1, b_1] times ...times [a_N, b_N]$上の点列$(x_n) = ((x_n^1, ....x_n^N))$が$x = (x^1, .....^N)$に収束したとすると、
各$i = 1, ... N$に対して$a_i <= x_n^i <= b_i$なので$a_i <= x^i <= b_i$、つまり極限$x = (x^1, ....x^N)$は$[a_1, b_1] times .....mes [a_N, b_N]$の元である。
したがって、$[a_1, b_1] times ...times [a_N, b_N]$は閉集合である。
...
#proof[
$(x_n) = ((x_n^1, ... x_n^N))$を有界閉矩形$[a_1, b_1] times ....imes [a_N, b_N]$上の点列とする。
...
これを繰り返していけば点列$(x_n)$のある部分列$(x_(n_k))$であってある点$x = (x^1, ... x^N)$に収束するものが取れる。
]
...
#proof[
最大について示す。
$f$の$X$上の上限を$l in macron(RR)$とおくと、$X$が空集合でないので$l != -oo$であり、$f(a_n) -> l$となる$X$上の点列$(a_n)$が存在する。
...
この時、ある$l > 0$が存在して曲線上のすべての点$c(t) = (x_1, ... x_N)$に対して、$[x_1-l, x_1+l] times ....imes [x_N-l, x_N+l] subset X$が成り立つ。
特にそのような二点は$x_1, ... x_N$軸に並行ないくつかの線分で結ばれる。
] <t_pcurv>
...
ここで$n = 1, 2, 3, ...に対して$delta = n^(- 1)$として取れば、$x_n, y_n in X$が存在して$d(x_n, y_n) < n^(-1)$, $abs(f(x_n)-f(y_n)) >= epsilon$が成り立つ。
...
この時、$N$次元空間上の関数$tilde(f)$を$x in RR^N$に対して
$
tilde(f)(x) = inf_(a in X) (f(a)+L d(a, x))
$
で定めるとき、
関数$tilde(f)$はすべての$x in X$に対して$tilde(f)(x) = f(x)$を満たす$RR^N$上のリプシッツ連続関数でそのリプシッツ係数は$L$である。
]

#proof[
まず、任意の$x, y in RR^N$と$epsilon > 0$に対して$tilde(f)(x) >= f(a)+L d(a, x)-epsilon$となる点$a in X$が存在するので、$tilde(f)(x)$でも同じ$a$を取れば三角不等式より、
$
tilde(f)(y)-tilde(f)(x) <= L d(a, y)-L d(a, x)+epsilon <= L d(x, y)+epsilon.
$
ここから$tilde(f)$は$RR^N$上のリプシッツ連続関数でリプシッツ定数は$L$以下であることがわかる。
次に$x in X$に対して$a$として$x$を取れば
$
tilde(f)(x) <= f(x)+L d(x, x) = f(x)
$
が成り立ち、
$f$のリプシッツ連続性を使えば任意の$a in X$に対して
$
f(a)+L d(a, x) >= f(x)
$
なので$tilde(f)(x) >= f(x)$も成り立つ。
したがって$x in X$で$tilde(f)(x) = f(x)$であり、$tilde(f)$のリプシッツ係数は$L$である。
]
