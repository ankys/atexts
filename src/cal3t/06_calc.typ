...
$
D_N (a) = { (x_1, ... x_N) in RR^N mid(|) abs(x_1)+....bs(x_N) <= a }
$
として定義される。
...
$
B_N (a) = { (x_1, ... x_N) in RR^N mid(|) x_1^2+...._N^2 <= a^2 }
$
の体積を考える。
...
vol(B_N (a))
= cases(
	(2^M pi^(M-1))/((2 M-1)(2 M-3)...3 dot 1) a^(2 M-1) &quad (N = 2 M-1),
	(2^M pi^M)/((2 M)(2 M-2)...4 dot 2) a^(2 M) &quad (N = 2 M)
)
$
がわかる。
...
// $
// I = sum_(n = 1)^oo 1/n^2 = 1/1+1/4+1/9+...
// $
// を考える問題である。
