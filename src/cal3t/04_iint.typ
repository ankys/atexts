...
それと同様のことを$N$次元有界閉矩形$[a_1, b_1] times ...times [a_N, b_N]$上の有界な$N$変数関数$f(x_1, ....x_N)$について考える。
各区間$[a_i, b_i]$を分割$a_i = p^i_0 <= p^i_1 <= p^i_2 <= p^i_3 <= ...<= p^i_(K_i) = b_i$してその分割を$P_i$とおく。
_上リーマン和_と_下リーマン和_は次によって定義される。
$
overline(S)(f, P_1, ... P_N) & = sum_(k_1 = 1)^(K_1) ....um_(k_N = 1)^(K_N) sup_(x_1 in [p^1_(k_1-1), p^1_(k_1)]) .....p_(x_N in [p^N_(k_N-1), p^N_(k_N)]) f(x_1, ......N) (p^1_(k_1)-p^1_(k_1-1)) ... ..._(k_N)-p^N_(k_N-1)), \
underline(S)(f, P_1, ... P_N) & = sum_(k_1 = 1)^(K_1) ....um_(k_N = 1)^(K_N) inf_(x_1 in [p^1_(k_1-1), p^1_(k_1)]) .....f_(x_N in [p^N_(k_N-1), p^N_(k_N)]) f(x_1, ......N) (p^1_(k_1)-p^1_(k_1-1)) ... ..._(k_N)-p^N_(k_N-1)).
$
このリーマン和は一変数の場合と同様に大小関係について良い性質を持っており、上リーマン和の$P_1, ... P_N$についての下限を上積分、下リーマン和の上限を下積分としてそれらが一致するとき$f$は有界閉矩形$[a_1, b_1] times ....imes [a_N, b_N]$上で_重積分可能_あるいは_重可積分_または重積分は収束するといい値を_重積分_
$
integral ...integral_([a_1, b_1] times ....imes [a_N, b_N]) f(x_1, ....._N) dd((x_1, ......N))
$
と表す。
この時の$[a_1, b_1] times ...times [a_N, b_N]$を_積分領域_、$f$を_被積分関数_と呼ぶ。

定義としては上記の通りであるがいちいち成分を書いていたら大変なのでいくつかの記号を導入して記述を短縮する。
まず、$x = (x_1, ... x_N)$として、積分領域を$D = [a_1, b_1] times ....imes [a_N, b_N]$と被積分関数を$f(x)$と表す。
分割$P_1, ... P_N$に対して$P = (P_1, ....P_N)$とおくと$P$は積分領域$D = [a_1, b_1] times .....mes [a_N, b_N]$を$K_1 ......$個に分割したものと考えられ、
分割の一つの部分を$C = [p^1_(k_1-1), p^1_(k_1)] times ...times [p^N_(k_N-1), p^N_(k_N)]$とおいて、
その$N$次元_面積_を
$
area(C) = (p^1_(k_1)-p^1_(k_1-1)) ...(p^N_(k_N)-p^N_(k_N-1))
$
とする。
...
$
が成り立つ。
特に各積分が収束するならば$N$変数関数$f(x_1, ... x_N)$について
$
integral ...integral_([a_1, b_1] times ....imes [a_N, b_N]) f(x_1, ....._N) dd((x_1, ......N))
= integral_(a_1)^(b_1) ...integral_(a_N)^(b_N) f(x_1, ....x_N) dd(x_N) .....(x_1)
$
が成り立つ。
]
...
$
を考える。
これを重積分の定義に従って計算すると、特殊な分割として二つの区間$[0, 1]$を$K = 1, 2, 3, ...等分して$k, l = 1, ....K$に対して$(x, y) = (k/K, l/K)$での値を考えることで
$
I
...
$P$を$D$の分割、つまり$D$の面積確定有界閉部分集合$E$の集合で、任意の$E_1, E_2 in P$, $E_1 != E_2$に対して$E_1 inter E_2$は面積零集合で$union.big_(E in P) E = D$を満たすとする。
...
この時、$A = mat(a_1, ...c, a_N)$として、$D$は$N$個の$N$次元ベクトル$a_1, ....a_N$が張る平行四辺形（の$N$次元版）であり、示すべき式は
$
area(D) = abs(det A)$
となる。
このことの証明は省略する。
...
次に一般の$C^1$級変換$x = Phi (u)$と$Phi^(-1) (D)$が点$P_0 (u_1, ... u_N)$を含み一辺の長さが$h > 0$の正方形（の$N$次元版）$R_(u, h) = [u_1, u_1+h] times ....imes [u_N, u_N+h]$の場合について、
$
...
$n = 0, 1, 2, 3, ...に対して、全空間$RR^N$を一辺の長さが$h = 2^(-n)$の$N$次元正方形$R_(u, h)$ ($u = (h i_1, ....h i_N)$, $i_1, ....._N in ZZ$)に分割する。
...
#theorem([ガウス・グリーンの定理])[
$D$を有界開集合の閉包として境界$partial D$は$C^1$級で$f(x) = f(x_1, ... x_N)$を$D$を含む開集合上の$C^1$級関数とする。
このとき各$i = 1, ... N$に対して
$
