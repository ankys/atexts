...
@r_cal2t では微分可能性が主な話題になっていたが、本テキストでは関数や場は必要な回数だけ微分可能であるという_滑らか_であることを前提として話を進める。
...
逆行列のような行列から行列への関数の勾配は本来四階のテンソルになる。
そこで変数行列の各成分についての偏微分を考える。

#example([恒等変換の偏微分])[
$M times N$行列$X = (x_(i j))$の$x_(i j)$偏微分は、$i j$成分だけが$x_(i j)$に依存するので、そこだけ$1$で他は$0$となる。
つまり、
$
partial_(x_(i j)) (X) = E_(i j)
$
である。
]

#example([転置行列の偏微分])[
$M times N$行列$X = (x_(i j))$から転置行列$X^T = (x_(j i))$へ移す転置の$x_(i j)$偏微分は、$j i$成分だけが$x_(i j)$に依存するので、そこだけ$1$で他は$0$となる。
つまり、
$
partial_(x_(i j)) (X^T) = E_(j i)
$
である。
]

#example([逆行列の偏微分])[
$N$次正方行列$X = (x_(i j))$の逆行列$X^(-1)$の$x_(i j)$偏微分は、逆行列の特徴づけ
$
X X^(-1) = I
$
の両辺を偏微分することで得られる。
左辺の偏微分は
$
partial_(x_(i j)) (X X^(-1))
= partial_(x_(i j)) (X) X^(-1)+X partial_(x_(i j)) (X^(-1))
$
であり、右辺は定数なので偏微分は$0$である。
したがって
$
partial_(x_(i j)) (X^(-1))
= -X^(-1) partial_(x_(i j)) (X) X^(-1)
= -X^(-1) E_(i j) X^(-1)
$
を得る。
]

以上のことを応用すると、次のような微分も計算できる。

#example[
$N$次正方行列$A, X$に対して逆行列との内積$A colon X^(-1) = tr(A^T X^(-1))$の$X = (x_(i j))$についての勾配を考える。
まず$x_(i j)$偏微分から考えると
$
partial_(x_(i j)) (A colon X^(-1))
&= partial_(x_(i j)) (tr(A^T X^(-1)))
= tr(A^T partial_(x_(i j)) (X^(-1))) \
&= tr(-A^T X^(-1) E_(i j) X^(-1))
= -tr(E_(i j) X^(-1) A^T X^(-1)) \
&= -E_(j i) colon (X^(-1) A^T X^(-1))
= -(X^(-1) A^T X^(-1))_(j i).
$
したがって勾配は
$
gradient_X (A colon X^(-1))
= -(X^(-1) A^T X^(-1))^T
$
となる。
]
...
$
となる。
ヘッセ行列については@r_cal3t ですでに紹介されていて、主に極値の判定に使われる。
