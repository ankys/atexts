...
$ <e_diag>
と対角化される。
さらに$K$が正規化可能な場合は$P$は正規直交行列として取れ、
$
A = P dmat(c_1, dots.down, c_N) P^+
$ <e_diag2>
が成り立つ。
]
...
$
とおき、$K$の_実部_と呼ぶ。
実部$Re(K)$は$K$の加法と乗法で閉じていて、$K$のいわゆる部分体になっている。
また、実部$Re(K)$に属するスカラーを_実_なスカラーと呼ぶ。
正値$a in K_+$は常に実である。

すべての成分が実な行列を_実行列_といい、必要に応じて実対称行列や実直交行列なども定義する。
これまでの内容でわかることはエルミート行列は（固有多項式が因数分解されるなら）実対角行列に対角化されるということである。
ここでの主題は実エルミート行列は実直交行列を使って実対角行列に対角化されるということである。
なお、実エルミート行列は実対称行列に他ならないことに注意する。
