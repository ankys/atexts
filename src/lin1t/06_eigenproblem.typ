...
(x I-A) bold(v) = bold(0)
$
の解であり、$bold(v) != bold(0)$でないと線形独立にならない。
...
$p+q != 0$の場合は$2$つの異なる固有値を持つので$A$は対角化可能である。
この時、固有値$x = 1$に対しては
$
x I-A = mat(p, -q; -p, q)
$
...
このとき固有空間について$W(c) inter W(d) = O_V$が成り立つ。
]

#proof[
$bold(v) in W(c) inter W(d)$とすると、
$
A bold(v) = c bold(v) = d bold(v).
$
よって、$c != d$より$bold(v) = bold(0)_V$を得る。
]
...
$
となるベクトル$bold(v)_1 != bold(0)$が存在する。
