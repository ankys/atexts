...
#import "/deps/physics.typ": Set, dd, dv, evaluated
...
区間$[a, b]$の_分割_$P$とは
$
a = p_0 <= p_1 <= p_2 <= p_3 <= ... <= p_N = b
$
を満たす点の並び$p_0, p_1, p_2, p_3, ..., p_N$のことである($N = 1, 2, 3, ...$)。
つまり、区間$[a, b]$を_小区間_$[p_0, p_1], [p_1, p_2], [p_2, p_3], ..., [p_(N-1), p_N]$に分割している。
また、_分割の大きさ_を小区間の大きさの中で最も大きいもの、つまり
$
abs(P) = max Set(p_n-p_(n-1); n = 1, 2, 3, ..., N)
$
とする。

ここで小区間$[p_(n-1), p_n]$ ($n = 1, 2, 3, ..., N$)において$x$軸と$y = f(x)$および$x = p_(n-1)$と$x = p_n$で囲まれた部分を考えると一般には長方形とは限らないが、分割の大きさが小さいと長方形とみなしても全体の面積を計算するのには差し支えないと考える。
...
$
を$f$の区間$[a, b]$の分割$p_0, p_1, p_2, p_3, ..., p_N$と代表点$t_1, t_2, t_3, ..., t_N$の_リーマン和_という。
...
特殊な分割として区間$[a, b]$の$N = 1, 2, 3, ...$等分つまり$p_n = a+n/N (b-a)$ ($n = 0, 1, 2, 3, ....N$)を代表点として小区間の右端点$t_n = p_n$ ($n = 1, 2, 3, .....$)をそれぞれ考えることにしよう。
この時、リーマン和は
$
sum_(n = 1)^N f(t_n)(p_n-p_(n-1))
...
区間$[a, b]$の分割$P$を$a = p_0 < p_1 < p_2 < p_3 < ...< p_N = b$とする。
...
分割$P'$を$a = p'_0 < p'_1 < p'_2 < p'_3 < ...< p'_M = b$として、
$
{ p_0, p_1, p_2, p_3, ... p_N } subset { p'_0, p'_1, p'_2, p'_3, ....p'_M }
$
...
#proof[
$n = 2, 3, ...$に対して$a_n = a+2^(-n) (b-a)$と$b_n = b-2^(-n) (b-a)$とおけば、
...
$
integral x^n dd(x) = 1/(n+1) x^(n+1)+C quad (n = 0, 1, 2, 3, ...,
quad integral x^n dd(x) = 1/(n+1) x^(n+1)+C quad (n =-2,-3, ... x != 0),
$
$
integral abs(x)^a dd(x) = 1/(a+1) x abs(x)^a+C quad (a !=-1, x != 0).
$
$
integral e^x dd(x) = e^x+C,
quad integral a^x dd(x) = 1/(log a) a^x+C quad (a > 0, a != 1).
$
$
integral 1/x dd(x) = log abs(x)+C quad (x != 0),
$
$
integral sin x dd(x) = -cos x+C,
quad integral cos x dd(x) = sin x+C,
quad integral 1/(cos^2 x) dd(x) = tan x+C quad (x != pi/2+n pi, n in bb(Z)).
$
$
...
quad integral 1/(1-x^2) dd(x) = 1/2 log abs((1+x)/(1-x))+C quad (x != plus.minus 1).
$
]
...
$
integral log abs(x) dd(x) = x log abs(x)-x+C quad (x != 0).
$
$
integral tan x dd(x) = -log abs(cos x)+C quad (x != pi/2+n pi, n in bb(Z)).
$
$
...
実数係数の有理式つまり実数係数の多項式$p(x) != 0$と実数係数の多項式$q(x)$の分数の形$(q(x))/(p(x))$になっている関数は積分が計算できる。
これは部分分数分解によりどの有理式も次の形の有理式の和として表されることによる。

- 単項式$a x^n$ ($a in RR$, $n = 0, 1, 2, 3, ...$)。
- 一次式の累乗$a/(x-b)^n$ ($a, b in RR$, $n = 1, 2, 3, ...$)。
- 判別式が負な二次式の累乗で次の形$(2 a (x-b))/(((x-b)^2+c^2)^n)$ ($a, b, c in RR$, $n = 1, 2, 3, ...$)。
- 判別式が負な二次式の累乗$a/((x-b)^2+c^2)^n$ ($a, b, c in RR$, $c != 0$, $n = 1, 2, 3, ...$)。

これらのうち一つ目と二つ目は$x^n$, $x^(-n)$の積分をそれぞれ使えばよく、
三つ目は$y = (x-b)^2+c^2$と置換することで$y^(-n)$の積分に帰着される。
四つ目は置換$y = (x-b)/c$により、
$
...
$
と計算される($x != plus.minus 1$)。
]

根号を含む関数$R(x, root(n, (a x+b)/(c x+d)))$（$R$は実数係数の有理式、$a, b, c, d in RR$, $a d-b c != 0$, $n = 1, 2, 3, ...$）も積分が計算できる。
...
この節では積分を含んだいくつかの有名な不等式を紹介し、応用として（本来の応用先ではない場合もあるが）様々な積分や式に対して評価を与える。

#proposition([連続関数に対する積分の大小関係])[
$f(x)$, $g(x)$を有界閉区間$[a, b]$ ($a < b$)上の連続関数とする。
...
#proof[
不等式自体は@t_int_order ですでに示したとおりである。
等号成立条件については、もし$f(c) < g(c)$つまり$e := g(c)-f(c) > 0$となる$c in [a, b]$が存在したとする。
このとき$f$と$g$は連続関数なので$c$の近傍で$g(x)-f(x) > e/2$となる。
この近傍を$[c-d, c+d]$ ($d > 0$)とすれば、
$
integral_a^b g(x) dd(x)-integral_a^b f(x) dd(x)
= integral_a^b (g(x)-f(x)) dd(x)
>= integral_(c-d)^(c+d) (g(x)-f(x)) dd(x)
>= integral_(c-d)^(c+d) e/2 dd(x)
= d e > 0
$
となるので、積分の値は厳密に大きくなる。
]
...
$
であるから、
円周率$pi$が$22/7 = 3.142857142857143 ...$より小さいことがわかる。
...
より、
$
22/7-1/630 = 3.141269841269841 ...
< pi <
22/7-1/1260 = 3.142063492063492 ...
$
という評価が得られる。
]
...
つまり接線の傾きを$k$とすれば$x in [a, b]$に対して
$
k(x-(a+b)/2)+f((a+b)/2) <= f(x) <= (f(b)-f(a))/(b-a) (x-a)+f(a)
$
が成り立つ。
...
右辺は特殊な分割
$
P_N = { a, a+1 (b-a)/N, a+2 (b-a)/N, a+3 (b-a)/N, ... a+(N-1) (b-a)/N, b}
$
に対するリーマン和の極限なので、
$
...
#example([調和数の漸近近似])[
調和数とは正の整数の逆数の和のことをいい、$n = 1, 2, 3, ...$項の和
$
H_n = sum_(k = 1)^n 1/k = 1+1/2+1/3+...1/n
$
を考える。
...
数値的には$gamma = 0.577215664901532 ...$であるが、他の有名な定数$e$や$pi$との関係性などわかっていないことが多い。
]

#example([階乗の対数漸近近似])[
$n = 1, 2, 3, ...$の階乗$n!$を階乗を用いない形で評価することを考える。
...
有界閉区間とは限らない区間上の連続関数あるいは単調関数（広義単調増加関数と広義単調減少関数）に対する積分である広義積分は区間を有界閉区間で近似することで定義される。
...
$
の両方が存在するとき、
$f$は$(a, b)$上で_広義積分可能_であるまたは_広義積分が収束する_という。
...
次の命題は値が具体的に計算できる広義積分として重要であり、
指数が$a = -1$のところで切り替わることは記憶するに値する。
...
$
が収束する時、
$f$は$(a, b)$上で_絶対広義積分可能_または広義積分が_絶対収束_するという。
]
