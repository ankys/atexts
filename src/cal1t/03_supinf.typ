...
上限について示す。
証明は有理数の切断による。
$A$は上に有界なので$A$の上界$m$が存在し、$A$の上界からなる集合$overline(U)$は空ではない。
さらに有理数の稠密性（@t_dense_rational）より$m$と$m+1$の間に有理数が存在するので、$A$の有理数である上界からなる集合$U$も空でない。
また$A$は空でないのである$l in A$が存在し、$l$より小さい有理数も存在するので、$L = QQ\\U$も空でない。
この時$L$は有理数の切断である。
実際、$A$の上界$b$に対して$b <= a$となる$a$も$A$の上界なので、$a in L$, $b in U$ならば$a < b$である。
さらに$L$から一つの有理数$l$を取り出すと、$l$は$A$の上界ではないのである$m in A$が存在して$l < m$であり、$L\\{l}$は有理数の切断ではなくなる。
...
$overline(L)$は上に有界かつ空でないことがすぐわかるので、先ほど示したように上限$a$が存在する。
