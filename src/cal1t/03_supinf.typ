...
- $A$の上界が存在する時、$A$は_上に有界_であるという。
- $A$の下界が存在する時、$A$は_下に有界_であるという。
...
集合$A$が上に有界でない場合$sup A = +oo$、下に有界でない場合$inf A = -oo$、$A$が空集合の場合$inf A = +oo$, $sup A = -oo$と書くことにする。
この時、任意の実数の集合$A$に対して拡大実数の範囲で常に$sup A$と$inf A$が定義できることになる。
