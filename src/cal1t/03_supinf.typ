...
この節では基本的に実数の集合の最大・最小を扱うが、実数の演算は必要なく大小関係だけで定義されるので拡張実数の範囲で定義しておく。

$A$を拡張実数の集合つまり$macron(RR)$の部分集合とする。
...
- 次の二つの条件を満たす拡張実数$a$が存在したら$a$は$A$の_最大元_といい、$max A$と表す。
  + 任意の$x in A$に対して$x <= a$が成り立つ。
  + $a in A$である。
- 次の二つの条件を満たす拡張実数$a$が存在したら$a$は$A$の_最小元_といい、$min A$と表す。
  + 任意の$x in A$に対して$x >= a$が成り立つ。
  + $a in A$である。
] <d_max>
...
- 任意の$x in A$に対して$x <= m$が成り立つような拡張実数$m$は$A$の_上界_であるという。
  $A$の上界からなる集合に最小元がある時、その最小元を$A$の_上限_といい$sup A$と表す。
- 任意の$x in A$に対して$x >= m$が成り立つような拡張実数$m$は$A$の_下界_であるという。
...
#proposition[
$A$を拡張実数の集合、$a$を拡張実数とする。

- $a$が$A$の上限であることは次の二つの条件が成り立つことと同値である。
  + 任意の$x in A$に対して$x <= a$が成り立つ。
  + 任意の拡張実数$b < a$に対してある$x in A$が存在して$b < x$が成り立つ。
...
上限・下限の利点は拡張実数の範囲では常に存在することである。

#proposition([実数の連続性、上限公理])[
拡張実数の集合$A$には上限と下限が拡張実数として存在する。
]
...
上限・下限は拡張実数の範囲では常に存在するが、応用上重要なのは実数の範囲での上限・下限の存在条件である。
...
上限・下限が存在しないもう一つの場合は$A$自身が空集合の場合で、この時$A$の上界からなる集合と下界からなる集合はともに拡張実数全体になってしまい、$A$の上限は負の無限大で下限は正の無限大となる。
...
#proof[
上限についてのみ示す。
拡張実数$m$を$B$の上界とすると、任意の$x in A subset B$に対して$x <= m$より、$m$は$A$の上界でもある。
...
#remark[
本命題は実数値関数のみを対象としているが、和が不定形にならなければ拡張実数値関数でも成立する。
]
...
#proof[
$A$が有界であるならば、上界$a$と下界$b$が存在し、任意の$x in A$に対して$b <= x <= a$となる。
...
平方根は逆関数の定理で導入した方が理論の統一感があり連続性や単調性もいえて便利であるが、ここでは上限・下限の応用として導入する。
...
非負の実数$a$に対して集合$A = Set(x >= 0; x^2 <= a)$を考える。
