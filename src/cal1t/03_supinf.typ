...
この節では基本的に実数の集合の最大・最小を扱うが、実数の演算は必要なく大小関係だけで定義されるので拡大実数の範囲で定義しておく。

$A$を拡大実数の集合つまり$macron(RR)$の部分集合とする。
...
- 次の二つの条件を満たす拡大実数$a$が存在したら$a$は$A$の_最大元_といい、$max A$と表す。
  + 任意の$x in A$に対して$x <= a$が成り立つ。
  + $a in A$である。
- 次の二つの条件を満たす拡大実数$a$が存在したら$a$は$A$の_最小元_といい、$min A$と表す。
  + 任意の$x in A$に対して$x >= a$が成り立つ。
  + $a in A$である。
] <d_max>
...
$n = 1, 2, 3, ...$個の元からなる集合に対して最大元が存在することが示されたとする。
ここで$n+1$個の元からなる集合$A = { a_1, a_2, a_3, ..., a_n, a_(n+1) }$について、
帰納法の仮定より$n$個の元からなる部分集合${ a_1, a_2, a_3, ..., a_n }$には最大元$a$が存在する。
...
#example[
区間$[a, b], (a, b]$の最大元は$b$であり、区間$[a, b], [a, b)$の最小元は$a$である。
...
- 任意の$x in A$に対して$x <= m$が成り立つような拡大実数$m$は$A$の_上界_であるという。
  $A$の上界からなる集合に最小元がある時、その最小元を$A$の_上限_といい$sup A$と表す。
- 任意の$x in A$に対して$x >= m$が成り立つような拡大実数$m$は$A$の_下界_であるという。
...
#proposition[
$A$を拡大実数の集合、$a$を拡大実数とする。

- $a$が$A$の上限であることは次の二つの条件が成り立つことと同値である。
  + 任意の$x in A$に対して$x <= a$が成り立つ。
  + 任意の拡大実数$b < a$に対してある$x in A$が存在して$b < x$が成り立つ。
...
上限・下限の利点は拡大実数の範囲では常に存在することである。

#proposition([実数の連続性、上限公理])[
拡大実数の集合$A$には上限と下限が拡大実数として存在する。
]

#proof[
上限について示す。
証明は有理数の切断による。
$A$の上界からなる集合$overline(U)$として$overline(L) = macron(R)\\overline(U)$とおく。
このとき、$a in overline(L)$と$b in overline(U)$に対して$overline(L), overline(U)$の定義から$m in A$が存在して$a < m$が成り立つので$a < b$が成り立つ。
そのため、$overline(L)$は切断のようになっており、実際$L = overline(L) inter QQ$は空集合または全体集合または有理数の切断である。
このうち空集合は$-oo$、全体集合は$+oo$に対応するので、$L$が有理数の切断である場合を考える。
さらに$L$から一つの有理数$a$を取り出すと、$a$は$A$の上界ではないのである$m in A$が存在して$a < m$であり、$L\\{a}$は有理数の切断ではなくなる。
したがって、有理数の切断$L$に対応する実数$a$が存在し、@t_real_cut より$L = (-oo, a) inter QQ$とできる。
特に、$U = [a, +oo) inter QQ$であり、任意の有理数$x, y$に対して$x in U^c$, $y in U$ならば$x < a <= y$が成り立つ。
したがって任意の$x in overline(U)^c$, $y in overline(U)$に対して$x <= a <= y$であり、$a$は$A$の上界からなる集合の最小元つまり$A$の上限である。

下限については、$A$の下界からなる集合$overline(L)$が最大元を持ち有理数の切断$L$が実数の要件を満たさないので少し工夫する必要がある。
$overline(L)$には上限$a$が存在する。
ここで、任意の$l in overline(L)$と$x in A$に対して$l <= x$が成り立つので、$a <= x$も成り立ち、$a$は$A$の下界である。
したがって、$a$は$A$の下界からなる集合の最大元つまり$A$の下限である。
]

#example[
区間$[a, b], (a, b], [a, b), (a, b)$の上限はいずれも$b$であり、下限は$a$である。
]

上限・下限は拡大実数の範囲では常に存在するが、応用上重要なのは実数の範囲での上限・下限の存在条件である。
そのために上限・下限が存在しない場合を二つ紹介する。
一つは実数の集合$A$に対して上界・下界が実数範囲で存在しない場合で$A$の上界・下界からなる集合が無限大のみからなるので上限は正の無限大で下限は負の無限大となる。
この場合を排除するために次の有界性の概念を導入する。

#definition([集合の有界性])[
$A$を実数の集合とする。

- $A$の上界が実数として存在する時、$A$は_上に有界_であるという。
- $A$の下界が実数として存在する時、$A$は_下に有界_であるという。
- 上に有界かつ下に有界の時$A$は_有界_であるという。
] <d_bdd>

上限・下限が存在しないもう一つの場合は$A$自身が空集合の場合で、この時$A$の上界からなる集合と下界からなる集合はともに拡大実数全体になってしまい、$A$の上限は負の無限大で下限は正の無限大となる。
...
- もし$A$が上に有界かつ空でないならば$A$の上限が実数として存在する。
- もし$A$が下に有界かつ空でないならば$A$の下限が実数として存在する。
- もし$A$が有界かつ空でないならば$A$の上限と下限が実数として存在する。
]
...
#proof[
上限についてのみ示す。
拡大実数$m$を$B$の上界とすると、任意の$x in A subset B$に対して$x <= m$より、$m$は$A$の上界でもある。
...
ここで$A$のうち$N$以上$M$以下のものに制限して得られる集合$A inter { N, ..., M }$は高々$M - N+1$個の元からなる有限集合なので@t_maxfin より最大元$a$が整数として存在する。
この時、任意の$x in A inter { N, ..., M }$に対して$x <= a$かつ$a in A$かつ$N <= a <= M$である。
...
#example[
極端な例として、定数関数$f(x) = c$がある。
この関数の像は$c$のみからなる集合なので、最大値も最小値も上限も下限も$c$であり、任意の$x in X$が最大点かつ最小点である。
]

この上限・下限の性質として次が重要である。

#proposition([大小関係と上限・下限])[
$f$, $g$を集合$X$上の拡大実数値関数として、
任意の$x in X$に対して$f(x) <= g(x)$を満たしているとする。
このとき、
$
sup f <= sup g,
quad inf f <= inf g
...
#proof[
上限についてのみ示す。
任意の$x in X$に対して$f(x) <= g(x) <= sup g$なので、$sup g$は$f(X)$の上界である。
ここで$sup f$は$f(X)$の上界からなる集合の最小元なので、$sup f <= sup g$である。
下限についても同様なので証明は省略する。
]
...
#remark[
本命題は実数値関数のみを対象としているが、和が不定形にならなければ拡大実数値関数でも成立する。
]
...
実数（または拡大実数）$a$に対して$a$と$-a$のうち大きい方の値を$a$の_絶対値_といい$abs(a)$で表す。
...
絶対値を用いると実数の有界性を次のように特徴づけることができる。

#proposition[
実数の集合$A$が有界であることと、ある実数$M$が存在して任意の$x in A$に対して$abs(x) <= M$が成り立つことは同値である。
] <t_bdd_abs>

#proof[
$A$が有界であるならば、実数の上界$a$と下界$b$が存在し、任意の$x in A$に対して$b <= x <= a$となる。
ここで$M = max{ abs(a), abs(b) }$とおけば、$M >= -b$かつ$M >= a$なので$-M <= x <= M$であり、$abs(x) <= M$である。
逆に$abs(x) <= M$ならば$-M <= x <= M$なので、$A$は有界である。
]

== 平方根と複素数の絶対値

平方根は逆関数の定理を用いて導入した方が理論の統一感があり連続性や単調性もいえて便利であるが、ここでは上限・下限の応用として導入する。
...
非負の拡大実数$a$に対して、集合$A = Set(x >= 0; x^2 <= a)$の上限を$a$の_平方根_といい$sqrt(a)$で表す。
この集合$A$は$0 in A$なので空でなく、任意の$x in A$に対して$x >= 1$なら$x <= x^2 <= a$より$max { 1, a }$が上界になっている。
したがって、$sqrt(a)$は（$a$が非負の前提のもとで）非負値の拡大実数であり、実数の連続性より$a$が有限のときは$sqrt(a)$も有限の実数である。
また、$a = oo$のときは$sqrt(a) = oo$、つまり本テキストにおいては$sqrt(oo) = oo$とすることを認めている。

平方根の重要な性質は$sqrt(a) = 0$と$a = 0$が同値であることである。
実際、$a = 0$ならば、@t_ineq_square より$A = { 0 }$なので、$sqrt(a) = 0$である。
...
#proof[
$a >= 0$のとき、$A = Set(x >= 0; x^2 <= a^2) = [-a, +a] inter [0, oo] = [0, a]$より、$sqrt(a^2) = a = abs(a)$である。
...
複素数には大小関係がないが、有界性は@t_bdd_abs に基づき定義される。

#definition([複素数の集合の有界性])[
$A$を複素数の集合とする。
もしある実数$M$が存在して任意の$x in A$に対して$abs(x) <= M$が成り立つ時、$A$は_有界_であるという。
]

また、複素数の絶対値に対しても三角不等式が成立する。

#proposition([複素数の絶対値の三角不等式])[
二つの複素数$z$と$w$に対して不等式
$
