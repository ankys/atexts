...
$
lim_(N -> oo) 1/N \#Set(n = 1, ..., N; a <= x_n < b) = b-a
$ <e_equidist>
が成り立つことである。
...
$
が成り立つことと同値である。
実際、こちらの方が強い条件になっていて、逆については任意の$epsilon > 0$に対して、ある$k = 1, 2, 3, ...$が存在して任意の自然数$n >= k$に対して$abs(a_n-a) < epsilon$, $abs(b_n-b) < epsilon$が成り立つことから、$N = 1, 2, 3, ...$に対して
$
...
この定理において$alpha$が無理数であることが非常に重要で、例えば$alpha$が有理数$1/p$、$beta = 0$の場合は$n$が$p$増えるともとの位置に戻るため、${alpha n+beta}$の取りうる値は${ 0/p, ..., (p-1)/p }$となり一様分布にはならない。
...
つまり任意の$0 <= a < b <= 1$に対して$a <= {alpha n+beta} < b$を満たす$n = 1, ...$が存在する。
]
...
まず、$N = ceil(2/(b-a))$とするとある$l = 0, ..., N-1$が存在して$a <= l/N < (l+1)/N <= b$とできることに注意する。
この$N = 1, 2, 3, ...$に対して次の議論をする。
...
例えば$n = 1, 2, 3, ...$に対して$2^n$の先頭の位の数$a_n$を考えてみよう。
$2^n$を並べて書き出すと
#quote(block: true)[
...
一般に正の整数（または正の実数）$y$について桁数を$l in ZZ$、先頭の位の数を$d = 1, ..., 9$とすると
$
...
#figure(
  align(center)[#table(
    ...
    [$2$], [$0.1760912590556813 ...$],
    [$3$], [$0.12493873660829985 ...$],
    [$4$], [$0.09691001300805646 ...$],
    [$5$], [$0.07918124604762478 ...$],
    [$6$], [$0.06694678963061318 ...$],
    [$7$], [$0.057991946977686726 ...$],
    [$8$], [$0.05115252244738144 ...$],
    [$9$], [$0.04575749056067513 ...$],
    [期待値], [$3.440236967123207 ...$],
  )]
 , caption: [先頭の位の数の分布]
 , kind: table
) <f_benford_law>
