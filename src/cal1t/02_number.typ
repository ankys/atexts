...
#proof[
$a = plus.minus oo$のときは$a^2 = +oo >= 0$であるので、$a$が実数のときを考える。
$a$の符号により場合分けする。
- $a >= 0$のとき、両辺に$a$をかけることで$a^2 >= 0 dot a = 0$が成立する。
- $a <= 0$のとき、$-a >= 0$なので、両辺に$-a$をかけることで$a^2 = (-a)^2 >= 0 dot (-a) = 0$が成立する。
等号$a^2 = 0$が成り立つとき、$a != 0$だとすると両辺を$a$で割ることで$a = 0$となり、結果的に$a = 0$が成り立つ。
]
