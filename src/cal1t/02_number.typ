...
ここで$-1$や$1.5$それに$sqrt(2)$などは自然数ではない。
なお、$0$を自然数とするかどうかは状況によってまちまちであるがこのテキストでは$0$も自然数として考える。
_自然数全体の集合_を$NN$で表す。
つまり、
$
NN = { 0, 1, 2, 3, ... }
...
このテキストでは抽象的な数列の添字として利用する場合のみに$NN$を用いて、添字を明示したい場合は$0, 1, 2, 3, ...$や$1, 2, 3, ...$などと書く。

_数学的帰納法_とは自然数$n$に関する命題$P(n)$の証明法である。
つまり、命題「任意の自然数$n = 0, 1, 2, 3, ...$に対して$P(n)$が成り立つ」を証明するためには以下の二つの命題を示せばよい。

- $P(0)$が成り立つ。
- 任意の自然数$n = 0, 1, 2, 3, ...$に対して$P(n) ==> P(n+1)$が成り立つ。

つまり最初の自然数で示して、その後一つずつ増やして証明していく論法である。
直接任意の自然数に対して証明できない場合でも数学的帰納法で証明できることが多い。
最初の自然数（上の場合は$0$）を取りかえることで証明できる自然数の範囲($n = 0, 1, 2, 3, ...$)を調節できるほか、
二つ目の条件の中の"$P(n)$"という部分をより示しやすく「任意の自然数$k = 0, 1, 2, 3, ..., n$に対して$P(k)$が成り立つ」に取りかえても同じ結論が得られる（累積帰納法）など様々な数学的帰納法の変種がある。
...
つまり、$a+x = b$を満たす自然数$x = 0, 1, 2, 3, ...$が存在する時$a <= b$と書き、そうでない時$a > b$と書く。
また、$a = b+x$を満たす自然数$x = 0, 1, 2, 3, ...$が存在する時$a >= b$と書き、そうでない時$a < b$と書く。

この大小関係の重要な性質として、以下がある。

#proposition([自然数の大小関係])[
...
_整数_はこれらの数$plus.minus 0, plus.minus 1, plus.minus 2, plus.minus 3, ...$をまとめてさらに$plus.minus 0$を$0$として同一視したものである。
整数のうち$+0, +1, +2, +3, ...$の部分は自然数$0, 1, 2, 3, ...$と同一視する。
_整数全体の集合_を$ZZ$で表す。
つまり、
$
ZZ
= { plus.minus 0, plus.minus 1, plus.minus 2, plus.minus 3, ... }
= { ..., -3, -2, -1, 0, 1, 2, 3, ... }
$
である。
...
_有理数_は二つの整数$n$, $m$ ($m != 0$)を使って分数$n\/m$や$n/m$の形で表される数のことであり、
...
有理数$a = n\/m$に対して$n != 0$の時、$m\/n$も有理数であり$a$の_逆数_といい$a^(-1)$と表す。
有理数のうち$0$と同一視されるものは$0\/m$ ($m != 0$)であることに注意すると、$0$以外の有理数には逆数が存在する。
...
+ （逆数）任意の有理数$a != 0$に対して$a dot x = x dot a = 1$が成り立つような有理数$x = a^(-1)$がただ一つ存在する。
]
...
例えば$a$を有理数とすると、$a$以下の有理数全体$Set(x in QQ; x <= a)$は有理数の切断である。
...
#proposition([実数の大小関係])[
+ （全順序）任意の実数$a, b$に対して$a <= b$または$a >= b$が成立し、両方が成立する場合は$a = b$に他ならない。
...
次に$a = b$つまり$A = B$であることを示す。
そのためには有理数$x$に対して$x in A$と$b in B$が同値を示せばよく、$x <= a$と$x in B$が同値を示せばよい。
これは$B$の定義に他ならないので、証明が完成した。
]

実数にも有理数同様、四則演算つまり加法、減法、乗法、除法と反数、逆数が定義できるがその説明をするには上限・下限の概念が必要になる。
ここではそれらが定義できたとしてその性質を紹介する。

#proposition([実数の演算と大小関係])[
...
+ （逆数）任意の実数$a != 0$に対して$a dot x = x dot a = 1$が成り立つような実数$x = a^(-1)$がただ一つ存在する。
...
$
である。
拡大実数$a$が与えられた時に、その数が正または負の無限大でないとき、つまり$a$が実数のときは$a$は_有限_であると表現する。
...
$
とする。
拡大実数の逆数は$(+oo)^(-1) = (-oo)^(-1) = 0$とするが$0$の逆数は定義されないことに注意する。
...
複素数$z = a+b i != 0$の逆数は$z^(-1) = a/(a^2+b^2)-b/(a^2+b^2) i$であり、除法は次になる。
$
...
+ （逆数）任意の複素数$a != 0$に対して$a dot x = x dot a = 1$が成り立つような複素数$x = a^(-1)$がただ一つ存在する。
]
...
不等式の基礎として_自乗_$a^2 = a dot a$に関する次の性質がある。

#proposition([自乗の単調性])[
任意の$0 <= a <= b$を満たす拡大実数$a, b$に対して、$a^2 <= b^2$が成り立つ。
等号成立条件は$a = b$である、つまり$a^2 = b^2$ならば$a = b$である。
]

#proof[
$b$が無限大のときは自乗$b^2$も正の無限大なので、不等号は成立する。
そのため、以降では$b$が有限のときを考え、このとき$a$も有限であることに注意する。
このとき、仮定$a <= b$の両辺に$a$をかけることで$a^2 <= a b$が成り立つ。
また、仮定$a <= b$の両辺に$b$をかけることで$a b <= b^2$が成り立つ。
以上より、$a^2 <= a b <= b^2$が成り立つので、$a^2 <= b^2$が成り立つ。
等号が成立する場合には$a^2 = a b = b^2$が成り立つ。
ここで、$b != 0$のときには、二つの等式$a b = b^2$の両辺を$b != 0$で割ると$a = b$が得られる。
$b = 0$のときは$a = b = 0$でやはり$a = b$が得られる。
]

#proposition([自乗の非負性])[
任意の拡大実数$a$に対して$a^2 >= 0$が成立する。
等号成立条件は$a = 0$である、つまり$a^2 = 0$ならば$a = 0$である。
] <t_ineq_square>

#proof[
$a$の符号により場合分けする。
- $a >= 0$ときは、自乗の単調性で$a = 0$、$b$を$a$とすることで、$a^2 >= 0$が成立する。
- $a <= 0$のとき、$-a >= 0$なので、$(-a)^2 = a^2 >= 0$が成立する。
]

なお、以上の内容をさらに詳しく考えると次のようにまとめることができる。

#proposition([二次不等式])[
$a$を非負の拡大実数とする。
このとき任意の拡大実数$x$に対して次が成り立つ。
- $x^2 <= a^2$が成り立つことと$-a <= x <= +a$が成り立つことは同値である。
- $x^2 >= a^2$が成り立つことと$x <= -a or x >= +a$が成り立つことは同値である。
- $x^2 = a^2$が成り立つことと$x = plus.minus a$が成り立つことは同値である。
]

#proof[
まず、自乗の単調性から$+0 <= x <= +a$に対して$x^2 <= a^2$で$-a <= x <= -0$に対しても$x^2 <= a^2$である。
さらにやはり自乗の単調性から$x >= +a$に対して$x^2 >= a^2$で$x <= -a$に対しても$x^2 >= a^2$が成り立つ。
また、等号$x^2 = a^2$が成り立つ条件は$x >= +0$のときは$x = +a$で$x <= -0$のときは$x = -a$なので、まとめると$x = plus.minus a$である。
ここで不等式$x^2 <= a^2$において$x > +a$も成り立つとすると$x^2 >= a^2$より等号が成立するので$x =plus.minus a$となり、$x > +a$に矛盾する。
同様に$x < +a$でも矛盾が導かれるので、命題の一つ目の内容が示される。
さらに不等式$x^2 >= a^2$と$-a < x < +a$が同時に成り立つとすると$x^2 <= a^2$で$x = plus.minus a$となり矛盾するので、二つ目の内容も示された。
]
...
コーシー・シュワルツの不等式は項数を増やしたり積分に一般化され、後の章で登場する。
