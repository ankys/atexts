...
#remark[
この定理は$a < b+epsilon$が成り立つ時でも使えるが結論は$a <= b$であって、その場合でも$a < b$は得られない。
...
#proof[
最初の内容のみ証明する。
背理法でもし$a > b$とすると$epsilon = (a-b)/2 > 0$と取ることができ、
$
a-b <= epsilon = (a-b)/2.
$
...
数列の極限は前節の$epsilon$論法によって次のように定義される。
...
- 実数列$(a_n)_(n in NN)$が$n -> oo$で_$+oo$に発散する_あるいは拡大実数列$(a_n)_(n in NN)$が$n -> oo$で_$+oo$に収束する_とは
  #quote(block: true)[
...
このことは自明のように思えるが、数列の添字の$n$は自然数なのに対して数列の項は実数であり、自然数と実数の差を埋める必要がある。

#proof([@t_limid の証明])[
$A > 0$とする。
天井を使って整数$N = ceil(A)+1$とおくことで、$N$は$N > A > 0$を満たす自然数である。
この時$N$以上の自然数$n$に対して$n >= N > A$より、数列$(n)_(n = 0)^oo$は$n -> oo$で$+oo$に発散することがわかる。
]
...
$
が成り立つ。
$N$未満の添字は$n = 0, ..., N-1$の有限個なので、
$
A = max { abs(a_0), ..., abs(a_(N-1)), abs(a)+1 }
$
とおくと$A$は実数で、任意の自然数$n$に対して$abs(a_n) <= A$である。
...
#remark[
証明でも触れるが、五つ目の商の極限について$b_n -> b != 0$だと十分大きいすべての$n$について$b_n != 0$であり、そのような$n$に対して$a_n/b_n$を考えることに注意する。
]
...
+ $epsilon > 0$を固定する。
  ...
  よって$a_n+b_n -> a+b$である。
+ $c = 0$の時、$c a_n = 0$,
  $c a = 0$なので$c a_n -> c a$であるため、$c != 0$の場合を考えればよい。
...
これらの計算は$plus.minus oo$に発散する（拡張実数の範囲で収束する）場合でもある程度拡張できる。
以下では特に重要な場合を示す。
...
+ $c > 1$の時、$c = 1+a$とする$a > 0$が取れ、二項定理より$n >= 1$において
  $
  c^n = (1+a)^n = 1+n a+binom(n, 2) a^2+...+binom(n, n) a^n >= n a.
  $
...
したがってこれを繰り返し用いることで、
$
abs(a_n) <= d abs(a_(n-1)) <= d^2 abs(a_(n-2)) <= ... <= d^n abs(a_0).
$
...
#definition([数列の単調性])[
$(a_n)_(n = 0)^oo$を拡大実数列とする。

- 任意の$n = 0, 1, 2, 3, ...$に対して$a_n <= a_(n+1)$が成り立つ、つまり
  $
  a_0 <= a_1 <= a_2 <= a_3 <= ...
  $
  のとき、$(a_n)$は_単調増加_という。
...
#theorem([単調収束定理])[
$(a_n)$を実数列とする。
- $(a_n)$が単調増加かつ上に有界のとき、$(a_n)$は実数の範囲で収束する。
- $(a_n)$が単調減少かつ下に有界のとき、$(a_n)$は実数の範囲で収束する。

$(a_n)$を拡大実数列とする。
- $(a_n)$が単調増加のとき、$(a_n)$は拡大実数の範囲で収束する。
- $(a_n)$が単調減少のとき、$(a_n)$は拡大実数の範囲で収束する。
] <t_mono_conv>

#remark[
証明を見ればわかるが実は極限は、$(a_n)$が単調増加のとき上限$sup_n a_n$で、$(a_n)$が単調減少のとき下限$inf_n a_n$である。
有界性の仮定は極限を実数の範囲に収めるための条件である。
]

#proof[
$(a_n)$が単調増加かつ上に有界の場合を証明する。
まず、$(a_n)$が上に有界であることから集合$Set(a_n; n in NN)$には上限$sup_n a_n$が有限の実数として存在する。
...
$(a_n)$が単調増加かつ上に有界でない場合は、$a_n -> +oo$を示す。
上に有界でないため任意の$A > 0$に対して、ある$N in NN$が存在して$a_N > +A$であり、$(a_n)$が単調増加であることから$N$以上の$n in NN$に対して$a_n > +A$なので、$a_n -> +oo$である。
...
$ <e_rec>
を考える。
これは_初期値_$a_0$が与えられたら、$n = 0, 1, 2, 3, ...$に対して$a_(n+1) = F(a_n)$で$a_0, a_1, a_2, a_3, ...$が（数学的帰納法から）求まり、数列$(a_n)_(n = 0)^oo$が定まりその極限を考える。
...
これによって、一般項を求めることが難しい問題でも極限を計算することができる（ことがある）。
...
$a_0 >= sqrt(c)$とするとき、算術平均・幾何平均の不等式から$n = 0, 1, 2, 3, ...$に対して
$
...
$
より、$(a_n)$は単調減少である。
よって単調収束定理より$(a_n)$は何らかの実数$a$に収束する。
$a$は極限方程式
$
a = 1/2 (a+c/a)
$
を満たし、これを解くと$a = plus.minus sqrt(c)$である。
ここで$a_n >= sqrt(c)$なので@t_limord より$a >= sqrt(c)$から$a = sqrt(c)$のみ適する。
...
$0 <= a_n <= 2$ならば$a_(n+1) = sqrt(2+a_n) >= sqrt(2) > 0$かつ$a_(n+1) = sqrt(2+a_n) <= sqrt(2+2) = 2$なので、$0 <= a_0 <= 2$とすると任意の$n = 0, 1, 2, 3, ...$に対して$0 <= a_n <= 2$で特に$(a_n)$は上に有界である。
さらに
$
...
#definition([上極限・下極限])[
$(a_n)_(n in NN)$を拡大実数列とする。

- 上限・下限を使って定義される次の拡大実数を$(a_n)$の$n -> oo$での_上極限_という。
  $
  limsup_(n -> oo) a_n = inf_(k in NN) sup Set(a_n; n in NN, n >= k).
  $
- 上限・下限を使って定義される次の拡大実数を$(a_n)$の$n -> oo$での_下極限_という。
  $
...
#proposition[
$(a_n)_(n in NN)$と$(b_n)_(n in NN)$を十分大きい任意の$n in NN$に対して$a_n <= b_n$を満たす拡大実数列とする。
この時、以下が成り立つ。
...
#proposition[
$(a_n)_(n in NN)$を拡大実数列とするとき、以下が成り立つ。
...
#proof[
上極限のみ示す。
$limsup_(n -> oo) a_n$が$+oo$でないつまり有限の実数の場合を証明する。
...
$a_n = (-1)^n a_0$となり$a_0 != 0$のとき$(a_n)$は発散する。
]
...
#proof[
三角不等式より$n = 1, 2, 3, ...$に対して、
$
abs((a_1+...+a_n)/n-a) = abs((a_1-a)+...+(a_n-a))/n <= (abs(a_1-a)+...+abs(a_n-a))/n
$
であることに注意する。
$epsilon > 0$に対して、$a_n -> a$であることから、$N$以上のすべての$n = 1, 2, 3, ...$に対して$abs(a_n-a) < epsilon$となるような$N = 1, 2, 3, ...$が存在するので、$n = N, ...$に対して、
$
abs((a_1+...+a_n)/n-a)
< (abs(a_1-a)+...+abs(a_N-a)+(n-N)epsilon)/n
= epsilon+(abs(a_1-a)+...+abs(a_N-a)-N epsilon)/n
$
である。
ここで$n -> oo$とすると最右辺は$epsilon$に収束するので、
$
limsup_(n -> oo) abs((a_1+...+a_n)/n-a) <= epsilon
$
...
$Set(1/n; n = 1, 2, 3, ...)$のような集合の場合は極限点は$0$のみでありそれ以外の点では極限を考えられないことに注意する。
...
関数$f$を有界閉区間$[a, b]$からそこへの連続関数とする（つまり$f([a, b]) subset [a, b]$である）。
...
この二つの定理はともに区間を左と右に二等分し、所望の点（中間値の定理ならば$x$、最大値最小値定理ならば最大点・最小点）が入る区間を選ぶという操作を繰り返し行い一点に縮小させるという論法で証明される。
次の補題はその議論を支えるものである。
...
まず、$a_0 = a$, $b_0 = b$とする。 $n = 0, 1, 2, 3, ...$に対して$a_n$, $b_n$が定義できた時$c_n = (a_n+b_n)/2$として、
...
$f(a) >= f(b)$の場合も同様に示される。
$min { f(a), f(b) } < y < max { f(a), f(b) }$の時は$x != a, b$なので$a < x < b$と取れる。
]
...
ある$n = 0, 1, 2, 3, ...$が存在して、任意の$x' in [a_n, b_n]$に対して$f(x') < f(x)+epsilon$である。
...
もし$f(x) = f(x') = y$となる2点$x, x' in [a, b]$が存在して$x != x'$だったとする。
...
連続関数は点$a$と$epsilon > 0$ごとに$delta > 0$の存在を要請していたが、
条件を強めて$delta$が$a$によらず一つで済む場合を一様連続という。

#definition([一様連続関数])[
$f$を実数の集合$X$上で定義された実数値関数として、$X$の点はすべて$X$の極限点であるとする。
ここで、任意の$epsilon > 0$に対して、ある$delta > 0$が存在して、$abs(x-y) < delta$を満たす任意の$x, y in X$に対して$abs(f(x)-f(y)) < epsilon$が成り立つ時、関数$f$は集合$X$上_一様連続_または$f$は$X$上の_一様連続関数_であるという。
]

一様連続関数は連続関数であることに注意する。

#example[
- 一次関数$f(x) = x$は$RR$上の連続関数であり一様連続関数でもある。
- 二次関数$f(x) = x^2$は$RR$上の連続関数であるが一様連続関数ではない。
  二次関数は有界閉区間$[a, b]$上では一様連続関数である。
]

一様連続性は今後の議論を展開する上で重要な性質であるが、連続関数と違って積で閉じているわけではなく確認が難しい。
しかしながらハイネ・カントールの定理などと呼ばれる次の定理が知られている。
...
ここで$n = 1, 2, 3, ...$に対して$delta = n^(-1)$として取れば、$x_n, y_n in [a, b]$が存在して$abs(x_n-y_n) < n^(-1)$, $abs(f(x_n)-f(y_n)) >= epsilon$が成り立つ。
...
度合い関数を用いれば連続性の条件を$epsilon$と$delta$を用いることなく簡潔に書くことができ便利であるが、
度合い関数を定義するために実一変数関数の連続性は別途定義する必要がある。
